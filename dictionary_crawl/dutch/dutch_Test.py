...
#         futures = []
#         for i, word in enumerate(words[:200], start=1):
#             word = word.split()[0]
...
#     with open("d:\\CS370\\dictionary_crawl\\dutch\\dutch10000-utf8.txt", "r", encoding="utf-8") as f:
...
def crawl_words(words):
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        futures = []
        for i, word in enumerate(words[:6540], start=1):  # Example limit to first 8 words
            word = word.split()[0]  # Treat only the first part of any line as the word
            print(f"Submitting task {i}/{len(words)}")
            futures.append(executor.submit(get_word_data, word))
            sleep(0.01)
...
if __name__ == "__main__":
    with open("d:\\CS370\\dictionary_crawl\\dutch\\filtered_dutch_words.txt", "r", encoding="utf-8") as f:
        words = f.read().splitlines()
    results = crawl_words(words)
    if results:
        with open("d:\\CS370\\dictionary_crawl\\dutch\\dutch_dict1.json", "w", encoding="utf-8") as f:
            json.dump(results, f, ensure_ascii=False, indent=4)
    else:
        print("No valid entries to save.")